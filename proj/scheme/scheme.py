...
#            if scheme_symbolp(vals.first):
#                child.bindings[formals.first] = self.lookup(vals.first)
#            else:
#                child.bindings[formals.first] = vals.first
                
#            elif scheme_numberp(vals.first):
#                child.bindings[formals.first] = vals.first
#            elif isinstance(vals.first, UserDefinedProcedure):
#                child.bindings[formals.first] = vals.first
#            else:
#                child.bindings[formals.first] = scheme_eval(vals.first, self)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 5A
        if scheme_atomp(expressions.second.first):
            env.bindings[expressions.first] = expressions.second.first
        else:
            val = scheme_eval(expressions.second.first, env)
            env.bindings[expressions.first] = val
        return expressions.first
        # END Question 5A
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN Question 9A
        expr = Pair('lambda', Pair(target.second, expressions.second))
        env.bindings[target.first] = scheme_eval(expr, env)
        return target.first
        # END Question 9A
    else:
...
def do_if_form(expressions, env):
    ...
    # BEGIN Question 13
    pred, cond = expressions.first, expressions.second
    if scheme_true(scheme_eval(pred, env)):
        return scheme_eval(cond.first, env)
    elif not cond.second is nil and scheme_listp(cond.second):
        return scheme_eval(cond.second.first, env)
    else:
        return okay
    # END Question 13
...
def make_let_frame(bindings, env):
    ...
    # BEGIN Question 16
    bindings.map(lambda pairs: check_form(pairs, 2, 2))
    formals = bindings.map(lambda pairs: pairs.first)
    vals = bindings.map(lambda pairs: scheme_eval(pairs.second.first, env))
...
def check_formals(formals):
    ...
    while not formals is nil:
        if not scheme_symbolp(formals.first):
            raise SchemeError("Formal shall contain only symbols")
        if formals.first in _symbols:
            raise SchemeError("Repeated symbols")
        if not scheme_listp(formals.second):
            raise SchemeError("Not well formed list")
        _symbols[formals.first] = 0
        formals = formals.second
    # END Question 11B
