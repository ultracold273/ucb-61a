...
def long_paths(tree, n):
    ...
    if tree.is_leaf():
        ...
        for b in tree.branches:
            paths += [Link(tree.entry, p) for p in long_paths(b, n - 1) if p]
        return paths
...
def partial_tree(s, n):
    ...
    if n == 1:
        ...
        tree_left, rest_left = partial_tree(s, left_size)
        tree_right, rest_right = partial_tree(rest_left.rest, right_size)
        return (Tree(rest_left.first, [tree_left, tree_right]), rest_right)
